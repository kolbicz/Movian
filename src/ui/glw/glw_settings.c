...
bing_images(screensaver_scanner_t *ss)
{
  ...
  snprintf(url, sizeof(url),
           "http://www.bing.com/HPImageArchive.aspx?format=js&n=8");
  ...
  if(list != NULL) {
    ...
    HTSMSG_FOREACH(f, list) {
      ...
      rstr_t *copyright = rstr_alloc(htsmsg_get_str(m, "copyright"));
      screensaver_add_item(rurl, copyright, ss);
      rstr_release(rurl);
      rstr_release(copyright);
    }
  }
  htsmsg_release(doc);
}

...
static int hevc_mp4toannexb_filter(AVBSFContext *ctx, AVPacket *out)
{
    ...
    while (bytestream2_get_bytes_left(&gb)) {
		...
		{
			if( 
				   (in->duration==102 &&  nalu_type == 39)// && ((buf_offset+2) < in->size) && in->data[buf_offset+2] == 4) // remove HDR10+			
				|| (in->duration==103 && (nalu_type == 62 || nalu_type == 63 )) // remove DolbyVision
				|| (in->duration==104 && (nalu_type == 62 || nalu_type == 63 || (nalu_type == 39 /*&& ((buf_offset+2) < in->size) && in->data[buf_offset+2] == 4*/))) // remove all
			)
			{
				bytestream2_skip(&gb, nalu_size);
				//buf_offset += nalu_size;
				continue;
			}
		}

...
/*
 * FILE_FULL_EA_INFORMATION (MS-FSCC 2.4.15), single entry.
 *
 * Used both to SET_INFO one extended attribute (ea_value_length == 0 deletes
 * an existing EA) and, on the wire, as the shape of what QUERY_INFO for this
 * class returns -- libsmb2 does not build/parse chained multi-entry EA lists,
 * only ever a single name/value pair per request.
 */
struct smb2_file_full_ea_info {
        uint8_t flags;
        uint8_t ea_name_length;
        uint16_t ea_value_length;
        const char *ea_name;        /* not NUL-terminated on this side */
        const uint8_t *ea_value;    /* ea_value_length bytes, may be NULL */
};

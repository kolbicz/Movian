...
/*
 * GetSessionFlags
 * Returns the SMB2_SESSION_FLAG_* bits (see smb2.h) reported by the server
 * in the session-setup response of the most recent successful session
 * establishment on this context (e.g. SMB2_SESSION_FLAG_IS_GUEST when the
 * server silently downgraded credentials to a guest session).
 */
uint16_t smb2_get_session_flags(struct smb2_context *smb2);

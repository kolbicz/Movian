...
read_more_data:
         ...
         *
         * Server role: reject a post-auth request that arrives unsigned on a
         * signing-required session (integrity bypass, MS-SMB2 3.3.5.2.4).
         * SMB2_ECHO is excluded: it is a no-op keepalive carrying no data or
         * state change, and Samba's smbclient sends its idle-keepalive echo
         * unsigned even on a signing-required session -- rejecting it would
         * tear down every idle interactive/mounted session for no security
         * gain (accepting an unsigned echo cannot bypass the integrity of any
         * real operation). SESSION_SETUP is excluded for the key-timing reason
         * above.
         */
        if (smb2_is_server(smb2) && smb2->sign &&
            smb2->hdr.command != SMB2_SESSION_SETUP &&
            smb2->hdr.command != SMB2_ECHO &&

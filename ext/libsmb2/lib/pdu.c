...
smb2_queue_pdu(struct smb2_context *smb2, struct smb2_pdu *pdu)
{
        ...
        for (p = pdu; p; p = p->next_compound) {
                ...
                /* MS-SMB2 3.2.5.2 / 3.3.5.5.3: fold this reply's final
                 * (post-signing) bytes into the server's preauth-integrity
                 * hash chain right here, while p is still guaranteed live.
                 * Doing this from the caller after smb2_queue_pdu() returns
                 * is unsafe: for the server role, smb2_add_to_outqueue()
                 * below can synchronously write the reply to a fast/local
                 * socket and free it via smb2_write_to_socket(), so a
                 * post-call smb3_update_preauth_hash(smb2, pdu->out.niov,
                 * ...) reads a freed (and iovector-zeroed) pdu, silently
                 * dropping the reply from the hash. That divergence between
                 * the client's and server's PreauthIntegrityHashValue only
                 * shows up as a wrong SMB 3.1.1 signing key, since 3.0/3.0.2
                 * do not fold the preauth hash into key derivation
                 * (Buksa/movian#74).
                 */
                if (smb2_is_server(smb2) &&
                    (p->header.command == SMB2_NEGOTIATE ||
                     p->header.command == SMB2_SESSION_SETUP)) {
                        smb3_update_preauth_hash(smb2, p->out.niov, &p->out.iov[0]);
                }
        }

                               ...
                               struct smb2_query_info_request *req)
{
        ...
        if (req->input_buffer_length > 0) {
                if (req->input_buffer == NULL) {
                        smb2_set_error(smb2, "input_buffer_length set but "
                                       "input_buffer is NULL");
                        return -1;
                }

                /* e.g. the FILE_GET_EA_INFORMATION name list a
                 * FILE_FULL_EA_INFORMATION query carries as its input buffer.
                 * Callers hand us fully-encoded wire bytes here -- there is
                 * no per-info-class shape to switch on like the set-info
                 * request has, just a raw blob to append after the fixed
                 * header. */
                buf = malloc(req->input_buffer_length);
                if (buf == NULL) {
                        smb2_set_error(smb2, "Failed to allocate query-info "
                                       "input buffer");
                        return -1;
                }
                memcpy(buf, req->input_buffer, req->input_buffer_length);
                iov = smb2_add_iovector(smb2, &pdu->out, buf,
                                        req->input_buffer_length, free);
                if (iov == NULL) {
                        smb2_set_error(smb2, "Failed to add iovector for "
                                       "query-info input buffer");
                        return -1;
                }
        }
                                 ...
                                 struct smb2_pdu *pdu)
{
        ...
        case SMB2_0_INFO_FILE:
                ...
                case SMB2_FILE_FULL_EA_INFORMATION:
                        /*
                         * No struct-based decode: callers that request this
                         * class (extended-attribute get) want the raw
                         * FILE_FULL_EA_INFORMATION bytes so they can walk the
                         * (NextEntryOffset, Flags, EaNameLength, EaValueLength,
                         * EaName, EaValue) layout themselves.
                         *
                         * output_buffer_length is a server-controlled header
                         * field while vec.len is what actually arrived in the
                         * PDU; never trust the former for sizing the copy.
                         * Copy MIN(vec.len, output_buffer_length) and clamp
                         * output_buffer_length down to what was copied so the
                         * consumer can never walk past the copied bytes.
                         */
                        if (rep->output_buffer_length > vec.len) {
                                rep->output_buffer_length = (uint32_t)vec.len;
                        }
                        ptr = smb2_alloc_init(smb2, rep->output_buffer_length);
                        if (ptr == NULL) {
                                smb2_set_error(smb2, "failed to allocate "
                                               "full ea info buffer");
                                return -1;
                        }
                        memcpy(ptr, vec.buf, rep->output_buffer_length);
                        break;
                case SMB2_FILE_ID_INFORMATION:
                        break;
                case SMB2_FILE_MODE_INFORMATION:

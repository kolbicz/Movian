               ...
               struct smb2_iovec *vec)
{
        ...
        do {
                ...
                ent->dirent.st.smb2_file_attributes = fs.file_attributes;
...
/* MS-SMB2 3.2.5.2
 * Not static: the server side must fold a queued NEGOTIATE/SESSION_SETUP
 * reply's bytes into the hash from inside smb2_queue_pdu() (pdu.c), before
 * smb2_add_to_outqueue()/smb2_write_to_socket() can synchronously free the
 * pdu -- see smb2_queue_pdu() in pdu.c for why (Buksa/movian#74).
 */
int
                 ...
                 void *command_data, void *private_data)
{
        ...
        smb2->session_flags = rep->session_flags;
           ...
           void *command_data, void *private_data)
{
        ...
        st->smb2_file_attributes = fs->basic.file_attributes;
...
uint16_t
smb2_get_session_flags(struct smb2_context *smb2)
{
        return smb2->session_flags;
}
...
smb2_tree_connect_request_cb(struct smb2_server *server, struct smb2_context *smb2, void *command_data, void *cb_data)
{
        ...
        if (!ret) {
                ...
                memset(&err, 0, sizeof(err));
                pdu = smb2_cmd_error_reply_async(smb2,
                                &err, SMB2_TREE_CONNECT, status, NULL, cb_data);
        }
        if (pdu != NULL) {
                smb2_set_pdu_message_id(smb2, pdu, smb2->message_id);
                smb2_queue_pdu(smb2, pdu);
...
smb2_create_request_cb(struct smb2_server *server, struct smb2_context *smb2, void *command_data, void *cb_data)
{
        ...
        req->tree_id = smb2->hdr.sync.tree_id;
        memset(&rep, 0, sizeof(rep));
...
smb2_lock_request_cb(struct smb2_server *server, struct smb2_context *smb2, void *command_data, void *cb_data)
{
        ...
        if (!ret) {
                ...
                memset(&err, 0, sizeof(err));
                pdu = smb2_cmd_error_reply_async(smb2,
                                &err, SMB2_LOCK, status, NULL, cb_data);
        }
        if (pdu != NULL) {
                smb2_set_pdu_message_id(smb2, pdu, smb2->message_id);
                smb2_queue_pdu(smb2, pdu);
...
smb2_query_directory_request_cb(struct smb2_server *server, struct smb2_context *smb2, void *command_data, void *cb_data)
{
        ...
        if (ret < 0) {
                uint32_t status = ret == -1 ? SMB2_STATUS_NOT_IMPLEMENTED : ret;
                pdu = smb2_cmd_error_reply_async(smb2,
                                &err, SMB2_QUERY_DIRECTORY, status, NULL, cb_data);
        }
        else if (!ret) {
...
smb2_session_setup_request_cb(struct smb2_context *smb2, int status, void *command_data, void *cb_data)
{
        ...
        smb2_set_pdu_message_id(smb2, pdu, smb2->message_id);
        smb2_queue_pdu(smb2, pdu);
        /* Preauth-hash update for this reply now happens inside
         * smb2_queue_pdu() (pdu.c), before the pdu can be freed by a
         * synchronous write -- see the comment there (Buksa/movian#74). */
}
...
smb2_negotiate_request_cb(struct smb2_context *smb2, int status, void *command_data, void *cb_data)
{
        ...
        if (req) {
                ...
                if (!server->allow_anonymous ||
                        ...
                        /* MS-SMB2 3.3.4.1.1: for an authenticated (non-anonymous)
                         * session on 2.1 or any 3.x dialect, signing is required
                         * whenever both sides have it enabled, regardless of
                         * whether the client asserted SIGNING_REQUIRED. This used
                         * to only cover 2.1 and >=3.1.1, leaving 3.0/3.0.2 clients
                         * that only assert SIGNING_ENABLED (e.g. samba's
                         * smbclient) with an unsigned, effectively unusable
                         * session (Buksa/movian#74).
                         */

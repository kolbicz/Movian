                             ...
                             struct smb2_set_info_request *req)
{
        ...
        case SMB2_0_INFO_FILE:
                ...
                case SMB2_FILE_FULL_EA_INFORMATION:
                         ...
                         * EaValueLength(2) + EaName + NUL(1) + EaValue.
                         * Single entry only -- see struct smb2_file_full_ea_info. */
                        len = 8 + eai->ea_name_length + 1 + eai->ea_value_length;
                        smb2_set_uint32(iov, 4, len); /* buffer length */
                        ...
                        iov = smb2_add_iovector(smb2, &pdu->out, buf, len,
                                                free);
                        if (iov == NULL) {
                                smb2_set_error(smb2, "Failed to add iovector for set-info full EA data");
                                return -1;
                        }

                        smb2_set_uint32(iov, 0, 0); /* next_entry_offset */
                        smb2_set_uint8(iov, 4, eai->flags);
                        smb2_set_uint8(iov, 5, eai->ea_name_length);
                        smb2_set_uint16(iov, 6, eai->ea_value_length);
                        memcpy(iov->buf + 8, eai->ea_name, eai->ea_name_length);
                        iov->buf[8 + eai->ea_name_length] = 0; /* NUL after name */
                        if (eai->ea_value_length > 0) {
                                memcpy(iov->buf + 8 + eai->ea_name_length + 1,
                                       eai->ea_value, eai->ea_value_length);
                        }
                        break;
                default:
